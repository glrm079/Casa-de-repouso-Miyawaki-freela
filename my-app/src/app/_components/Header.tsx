"use client"; // <- Adicione esta linha no topo

import { useState } from "react";

export function Header() {
    const [modal, setModal] = useState(false);
    const [fix, setFixed] = useState(setHeaderFixed);

    function setHeaderFixed(){
        document.getElementById('header')?.classList.add("fixed")
    }


    return (
        <section onScroll={()=>setFixed()} id="header" className="overflow-hidden bg-[#2c520c] shadow-md ">
            <div className="container mx-auto relative">
                <article className="flex flex-row justify-between items-center px-16 py-2 text-zinc-50">
                    <div>
                        <img className="w-14" src="/logo.webp" alt="Logo" />
                    </div>

                    <nav className="hidden md:flex flex-row items-center gap-4">
                        <a href="#">Home</a>
                        <a href="#">Sobre</a>
                        <a href="#">Serviços</a>
                        <a href="#">Blog</a>
                        <a className="bg-[#66a124] px-5 py-2 rounded font-semibold flex justify-center items-center" href="#">Entre em contato</a>
                    </nav>

                    {modal === false ? (
                        <svg 
                            onClick={() => setModal(true)} 
                            className="w-10 lg:hidden cursor-pointer" 
                            viewBox="0 0 24 24" 
                            fill="none" 
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path d="M20 7L4 7" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round"></path>
                            <path d="M20 12L4 12" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round"></path>
                            <path d="M20 17L4 17" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round"></path>
                        </svg>
                    ) : (
                        <h1 onClick={() => setModal(false)} className="cursor-pointer">Fechar</h1>
                    )}
                </article>
                {modal === true ? (
                    <div>
                    <nav className="flex flex-col text-zinc-50 items-center gap-4 p-3">
                       <a href="#">Home</a>
                       <a href="#">Sobre</a>
                       <a href="#">Serviços</a>
                       <a href="#">Blog</a>
                       <a className="bg-[#66a124] px-5 py-2 rounded font-semibold flex justify-center items-center" href="#">Entre em contato</a>
                   </nav>
               </div>):<>
               </>}
                    
            </div>
        </section>
        
    );
}
